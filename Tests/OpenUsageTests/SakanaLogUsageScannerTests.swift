...
final class SakanaLogUsageScannerTests: XCTestCase {
    private let now = OpenUsageISO8601.date(from: "2026-07-26T12:00:00.000Z")!
    ...
    func testAggregationKeepsUltraDeduplicatedAndFlagsUnpriceableFugu() throws {
        ...
        let scan = SakanaLogUsageScanner.aggregate(
            ...
            since: OpenUsageISO8601.date(from: "2026-07-01T00:00:00.000Z")!
        )
    ...
    func testParityAgainstRealLocalLogs() async throws {
        try XCTSkipUnless(ProcessInfo.processInfo.environment["OPENUSAGE_SAKANA_PARITY"] == "1")
    ...
    private func event(
        ...
        CodexLogUsageScanner.Event(
            timestamp: try XCTUnwrap(OpenUsageISO8601.date(from: timestamp)),
            model: model,
            input: input,
            cached: cached,
            output: output,
