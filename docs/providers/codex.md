...
Discovery checks the normal `~/.codex` and `~/.config/codex` homes, dot-directories in your home folder, directories directly under `~/.config`, and every explicit entry in `CODEX_HOME`. Runway accepts a comma-separated `CODEX_HOME` list for discovery; an individual Codex CLI process should still use one home at a time. For example:
