...
enum LocalUsageAPI {
    ...
    private struct WireSnapshot: Encodable {
        ...
        func encode(to encoder: Encoder) throws {
            ...
            try container.encode(RunwayISO8601.string(from: snapshot.refreshedAt), forKey: .fetchedAt)
        }
    }
    ...
    private struct WireLine: Encodable {
        ...
        func encode(to encoder: Encoder) throws {
            ...
            case .values(let label, let values, let color, let expiriesAt, _, _):
                ...
                try container.encodeIfPresent(expiriesAt.min().map(RunwayISO8601.string(from:)), forKey: .resetsAt)
            case .progress(let label, let used, let limit, let format, let resetsAt, let periodDurationMs, let color):
                ...
                try container.encodeIfPresent(resetsAt.map(RunwayISO8601.string(from:)), forKey: .resetsAt)
