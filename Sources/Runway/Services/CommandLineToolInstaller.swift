...
final class CommandLineToolInstaller {
    ...
    init(
        sourcePath: String = Bundle.main.bundleURL
            .appendingPathComponent("Contents/Helpers/runway").path,
        destinationPath: String = "/usr/local/bin/runway",
    ...
    func install() {
        ...
        guard status != .conflict else {
            errorMessage = "\(destinationPath) already exists and wasn't installed by Runway."
            return
        }
        guard fileManager.isExecutableFile(atPath: sourcePath) else {
            errorMessage = "The bundled terminal helper couldn't be found. Reinstall Runway and try again."
            return
        }
