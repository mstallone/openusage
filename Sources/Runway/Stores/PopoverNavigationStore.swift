...
/// The screen showing inside the menu-bar popover. Customize replaces the dashboard in place (the
/// popover has no window stack); Esc backs out to the dashboard first. Settings is NOT a popover
/// screen — it lives in its own standalone window (`SettingsWindowController`).
enum PopoverScreen: Hashable, Sendable {
    ...
    /// higher-ranked target enters from the trailing edge, a lower one from the leading edge.
    var slideRank: Int {
        switch self {
        case .dashboard: 0
        case .customize: 1
