...
struct SettingsScreen: View {
    ...
    /// macOS notification authorization for Runway, surfaced in the Notifications section so a
        ...
        return VStack(alignment: .leading, spacing: density.sectionSpacing) {
            ...
            if mountStage >= 1 {
                ...
                section("Privacy") {
                    ...
                    Text("While your screen is shared or recorded, the menu bar shows “Runway” instead of your usage.")
                        .font(.caption)
    ...
    private var commandLineSection: some View {
        section("Command Line") {
            ...
            Text("Adds a global `runway` command agents can use to monitor limits.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .padding(.horizontal, 12)
                .padding(.bottom, 8)
            ...
            if commandLineTool.status == .conflict {
                inlineNotice("\(commandLineTool.destinationPath) already exists and wasn't installed by Runway.")
    ...
    /// (`~/Library/Logs/Runway/Runway.log`); raising the level here applies live (no restart) and
