...
struct GrokAuthStore: Sendable {
    ...
    private func entryExpiresAt(_ entry: GrokAuthEntry) -> Date? {
        if let expiresAt = trimmed(entry.expiresAt), let date = RunwayISO8601.date(from: expiresAt) {
            return date
        }
        if let expires = trimmed(entry.expires), let date = RunwayISO8601.date(from: expires) {
            return date
        }
        return nil
    }
