...
/// Maps a pi session log's `provider` field to the Runway provider whose card its usage belongs on.
...
/// card of its own.
///
/// Only providers Runway already has a card for are listed. Pi providers with no Runway
/// equivalent are intentionally absent and left for future work:
/// - `nvidia-nim` — no Runway card.
///
...
enum PiProviderMapping {
    /// pi `provider` value → Runway `Provider.id`.
    ...
    /// The Runway card id for a pi provider, or nil when pi used a provider Runway doesn't track.
