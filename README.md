...
The iOS TestFlight job reuses the three `APPLE_NOTARY_*` secrets — no extra secrets — but needs one-time App Store Connect setup: create the app record (My Apps → New App, iOS, bundle ID `com.mattstallone.runway.mobile`, any unique SKU), and add an internal TestFlight tester group with **automatic distribution** so every uploaded build reaches testers without a manual step. The App ID must already carry the CloudKit capability with both Runway containers (see [iOS app](docs/ios-app.md)); the workflow's cloud signing then creates and refreshes the App Store provisioning profile on its own. External tester groups need Beta App Review and are not part of the automated flow.
